...
class FlowVAESurface(Module):
    ...
    def __init__(self, args):
        ...
        self.num_heads = 2
        self.MHSA = nn.MultiheadAttention(args.latent_dim + 2, self.num_heads)
        self.linear = nn.Linear(args.latent_dim + 2, args.latent_dim)
    ...
    def get_loss(self, x, view_angle, yaw, kl_weight, writer=None, it=None):
        ...
        z = torch.cat([z, view_angle, yaw], dim=1)  # (B, F+2)
        z = z.unsqueeze(0)  # (1, B, F+2)
        z, _ = self.MHSA(z, z, z)
        z = z.squeeze(0)  # (B, F+2)
        z = self.linear(z)  # (B, F)      
